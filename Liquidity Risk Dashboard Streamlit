# Liquidity Risk Reporting Dashboard – Streamlit Version (Production Enhanced – Basel III Alignment with Advanced Stress Logic)
...
if scenario == 'Adverse':
    filtered_df['Expected_Outflows'] *= 1.30  # 30% outflow spike
    filtered_df['Total_HQLA'] *= 0.85         # 15% haircut on HQLA
elif scenario == 'Severely Adverse':
    filtered_df['Expected_Outflows'] *= 1.50  # 50% outflow spike
    filtered_df['Total_HQLA'] *= 0.70         # 30% haircut on HQLA
...
if avg_lcr >= 120:
    st.success("✅ Excellent liquidity resilience. No action required.")
elif avg_lcr >= 100:
    st.info("ℹ️ Compliant – Maintain current buffer and monitor flows.")
elif avg_lcr >= 90:
    st.warning("⚠️ Close to threshold – Review inflows/outflows and prepare contingency lines.")
else:
    st.error("❌ Critical – Immediate liquidity stress. Escalate to Treasury Ops and activate stress protocol.")
